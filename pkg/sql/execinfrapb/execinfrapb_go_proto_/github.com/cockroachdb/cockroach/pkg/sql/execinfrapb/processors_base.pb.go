...
func (m *PostProcessSpec) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.OutputColumns) > 0 {
		...
		i -= l
		if l == len(m.OutputColumns) {
			dest := dAtA[i : i+len(m.OutputColumns)]
			for k, num := range m.OutputColumns {
				dest[k] = uint8(num)
			}
		} else {
			j1 := i
		...
		i = encodeVarintProcessorsBase(dAtA, i, uint64(uint64(l)))
		i--
		dAtA[i] = 0x1a
	}
	i--
...
func (m *Columns) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.Columns) > 0 {
		...
		i -= l
		if l == len(m.Columns) {
			dest := dAtA[i : i+len(m.Columns)]
			for k, num := range m.Columns {
				dest[k] = uint8(num)
			}
		} else {
			j2 := i
	...
	for iNdEx < l {
		...
		case 3:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.OutputColumns == nil {
						m.OutputColumns = make([]uint32, 0, elementCount)
					} else {
						m.OutputColumns = slices.Grow(m.OutputColumns, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v uint32
