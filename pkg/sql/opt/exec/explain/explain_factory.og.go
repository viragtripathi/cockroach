...
func (f *Factory) ConstructApplyJoin(
	...
	args := &applyJoinArgs{
		...
		RightSideForExplainFn: rightSideForExplainFn,
	}
...
func (f *Factory) ConstructInsert(
	...
	args := &insertArgs{
		...
		CheckCols:                   checkCols,
		UniqueWithTombstonesIndexes: uniqueWithTombstonesIndexes,
...
func (f *Factory) ConstructCreateTrigger(
	ct *tree.CreateTrigger,
) (exec.Node, error) {
	args := &createTriggerArgs{
		Ct: ct,
	}
	_n, err := newNode(createTriggerOp, args, nil /* ordering */)
	if err != nil {
		return nil, err
	}
	// Build the "real" node.
	wrapped, err := f.wrappedFactory.ConstructCreateTrigger(
		ct,
	)
	if err != nil {
		return nil, err
...
func (f *Factory) ConstructVectorSearch(
	table cat.Table,
	index cat.Index,
	...
	_n, err := newNode(vectorSearchOp, args, nil /* ordering */)
	if err != nil {
		return nil, err
	}
	// Build the "real" node.
	wrapped, err := f.wrappedFactory.ConstructVectorSearch(
		table,
		index,
		outCols,
		prefixConstraint,
...
func (f *Factory) ConstructVectorMutationSearch(
	input exec.Node,
	table cat.Table,
	index cat.Index,
	...
	_n, err := newNode(vectorMutationSearchOp, args, nil /* ordering */, inputNode)
	if err != nil {
		return nil, err
	}
	// Build the "real" node.
	wrapped, err := f.wrappedFactory.ConstructVectorMutationSearch(
		inputNode.WrappedNode(),
		table,
		index,
		prefixKeyCols,
...
func (f *Factory) ConstructUpdateSwap(
	input exec.Node,
	table cat.Table,
	fetchCols exec.TableColumnOrdinalSet,
	updateCols exec.TableColumnOrdinalSet,
	...
	_n, err := newNode(updateSwapOp, args, nil /* ordering */, inputNode)
	if err != nil {
		return nil, err
	}
	// Build the "real" node.
	wrapped, err := f.wrappedFactory.ConstructUpdateSwap(
		inputNode.WrappedNode(),
		table,
		fetchCols,
		updateCols,
...
func (f *Factory) ConstructDeleteSwap(
	input exec.Node,
	table cat.Table,
	fetchCols exec.TableColumnOrdinalSet,
	returnCols exec.TableColumnOrdinalSet,
	...
	_n, err := newNode(deleteSwapOp, args, nil /* ordering */, inputNode)
	if err != nil {
		return nil, err
	}
	// Build the "real" node.
	wrapped, err := f.wrappedFactory.ConstructDeleteSwap(
		inputNode.WrappedNode(),
		table,
		fetchCols,
		returnCols,
...
type vectorMutationSearchArgs struct {
	Input          *Node
	Table          cat.Table
	Index          cat.Index
	PrefixKeyCols  []exec.NodeColumnOrdinal
